...
구현이 진행 중이며, 해당 항목은 본문에 표시했다. 여기서는 RAG API 단독으로 가능한 구성만
다루고, LibreChat 등 챗 서비스와 연계했을 때 추가되는 부분은 [§6](#6-librechat-연계-시)에서
간단히 짚는다.
...
[무슨 일이 있었나]        Langfuse    ── 검색·MCP 호출 trace (저장소: ClickHouse)
...
| Langfuse | 요청 단위 추적 — 검색 지연, MCP 툴 호출, 품질 점수 | 제품 OTel 계측 |
| Grafana | 성능·안정성·품질 대시보드 | ClickHouse(Langfuse 데이터) + Prometheus |

## 2. Langfuse — 검색·MCP 추적 (제품 내장 기능)
...
| MCP 툴 호출 | 요청 `_meta.traceparent`를 수신해 상위 trace의 child로 기록 — MCP 클라이언트의 대화 trace 아래에 검색이 나타남 |

### 2.2 품질 평가 (LLM-as-judge, 선택)
...
| 품질 | LLM-as-judge 점수 추이(§2.2), 사용자 피드백(좋아요/싫어요) | 환경 구성·동작 확인 완료 |
...
| 표준 (권장) | + Langfuse 트레이싱 활성화 (§2.1) + Grafana 대시보드 | 검색 성능 분석, 요청 단위 진단 |

기본 수준은 설치 당일 구성 가능하다. 표준은 Langfuse 배포(또는 기존 인스턴스)가 전제이다.
구성 패키지 일정은 [지원 정책](../support/support-policy.md) 채널로 문의한다.

## 6. LibreChat 연계 시

LibreChat 등 MCP 클라이언트와 연계하면 사용자 질문 하나가 LLM 호출과 RAG 검색을 포함한
단일 trace로 이어져, **"느린 응답의 원인이 LLM인지 검색인지"를 trace 하나로 판별**할 수
있고 모델별 토큰·비용 대시보드도 추가로 구성할 수 있다. trace 전파에는 계측 패치 이미지가
필요하다(사내 검증 완료 — 적용은 지원 채널로 문의). 자세한 연동 방법은 LibreChat 연동
가이드(마이그레이션 예정)를 참고한다.
