...
제품 데모 영상의 촬영 스크립트다. 아래 시나리오를 `02-record.py`(`make record`)가 그대로
따라가며 자동 녹화한다.
...
| 2 | john@cnapcloud.com이 실제 수신 확인 가능한 메일함인지 확인 | 시나리오 1의 7번(멤버 초대)은 실제 이메일 발송을 유발함 (01-scenario.md 촬영 메모의 opt-in 항목과 동일 이슈) |
| 3 | john 역할(초대받은 일반 멤버)에 KB 생성 권한이 있는지 확인 | 시나리오 2의 3번(kb-04 생성)이 그 권한으로 가능한 동작인지 사전 확인 필요 — 안 되면 해당 단계에서 실패 |
| 4 | 촬영 시점에 다른 인제스트가 동시에 돌고 있지 않은지 확인 | 시나리오 2의 6번(Dagster run 클릭)이 "가장 최근 run = 방금 올린 ai_chat_소개.pdf"라는 전제에 의존 (01-scenario.md 컷 3과 동일 주의사항) |
...
`02b-record-namuwiki.py`가 독립적으로 녹화한다 (`make record-namuwiki`). rag-admin
